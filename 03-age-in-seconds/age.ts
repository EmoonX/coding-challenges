...
function calculateInYear(start: Date, end: Date): number {
	...
	if (start.getMonth() == end.getMonth()) {
		...
		const startMonthFullDays =
			getTotalDaysInMonth(start) - start.getDate();
		seconds += startMonthFullDays * SECONDS_PER_DAY;
		const date = start;
		const initialMonth = start.getMonth() + 1;
		for (let month = initialMonth; month < end.getMonth(); month++) {
			date.setMonth(month);
...
function calculate(birth: Date, now: Date): number {
	...
	const initialYear = birth.getFullYear() + 1;
	for (let year = initialYear; year < now.getFullYear(); year++) {
